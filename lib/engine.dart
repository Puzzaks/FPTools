...
class fastEngine extends HttpOverrides with material.ChangeNotifier{
  material.TextEditingController dropController = material.TextEditingController(text: "");
  ...
  List labelDomains = [];
  List selectedLabels = [];
  String globalPassword = md5.convert(utf8.encode(md5.convert(utf8.encode("PASSWORD LOL")).toString())).toString();
  ...
  Map availCPs = {
    "cluster1": "key1",
    "cluster2": "key2"
  };// I have not yet finalized CP editing and other functionality, WiP
  String selectCP = "cluster1";
  Map clusters = {};
  Map allClusters = {};
  ...
  Future<Map>       checkNumber(num) async {
    ...
    for(int i=0;i<numbers.length;i++){
      int number = int.parse(numbers[i].trim().replaceAll(RegExp(r'[^0-9]'), ''));
      numCheckStatus = "Checking $number on REDACTED (1/2)";
      ...
      await checkUNIFixed(number).then((value) async {
        numCheckStatus = "Checking $number on REDACTED (2/2)";
        ...
        await checkUNIHLR(number).then((value) async {
          ...
          await checkCP(number).then((value) async {
            ...
            if(checks[number]["UNIFixed"]["error"] == 1 && checks[number]["UNIHLR"]["error"]==1){
              await logAdd("REDACTED is unavailable for HLR", "error", "voip", false);
  ...
  Future<Map>       checkCP(number) async {
    var headers = {
      "Authorization": "API_KEY"//hardcoded key is bad but it was temporary untill it wasn't
    };
  ...
  Future<Map>       checkUNIHLR(number) async {
    ...
    var body = {
      "token": "API_KEY",//hardcoded key is bad but it was temporary untill it wasn't
      "number": number.toString()
    };
    var endpoint = "REDACTED.co";
    var method = "";
    try {
      final response = await http.post (
          Uri.https(
